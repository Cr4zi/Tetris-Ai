...
class TetrisNetwork:
    def __init__(self, env: Tetris, load=False, epsilon=1, epsilon_min=0.05, epsilon_decay=0.01, learning_rate=3e-4):
    ...
    def optimize_model(self):
        ...
        non_final_mask = torch.sensor(tuple(map(lambda s: s is not None, batch.next_state)), device=self.device, dtype=torch.bool)
    ...
    def train(self):
        ...
        for episode in range(episodes):
            ...
            print(f"At episode: {episode}, steps_done: {self.steps_done}, done: {self.env.done}")
            while not self.env.done:
                ...
                self.exp_buffer.push(state, action, next_state, reward)
